...
/// <summary>
/// Crisp vector icons drawn as stroked Paths on a 24-unit viewBox — the Avalonia port of the
/// WPF Icons.cs, same path data (mirrors site/index.html svg paths), so app and site read as
/// one brand. Keep the Data table in sync with the WPF file until the Phase 6 cutover retires it.
/// </summary>
public static class Icons
{
    ...
    // Fully qualified: bare "Control" resolves to the Wsnap.Control namespace (the linked
    // control-layer files), not Avalonia.Controls.Control.
    public static Avalonia.Controls.Control Make(string key, double size, IBrush stroke, double weight = 2.0)
    {
        var path = new Path
        {
