...
/// <summary>
/// The tray App is the resident host (port of the WPF App.Control.cs): it lends the command bus
/// its interactive/recording/UI abilities (overlay drag, GIF/video/scroll, thumbnails). Headless
/// commands go straight to <see cref="CaptureCore"/>; only what genuinely needs the UI thread
/// lands here. All entry points marshal onto the Avalonia dispatcher, so an incoming pipe/MCP
/// request runs on the UI thread safely.
/// </summary>
public partial class App : IResidentHost
{
    ...
    /// (drag end, recording stop, etc.). Posted + bridged through a TCS so the caller gets one await
    /// regardless of which thread the pipe/MCP request arrived on.</summary>
    public Task<CommandResult> ExecuteInteractiveAsync(WsnapCommand cmd, CancellationToken ct)
    {
        var tcs = new TaskCompletionSource<CommandResult>(TaskCreationOptions.RunContinuationsAsynchronously);
        Dispatcher.UIThread.Post(async () =>
        {
            try { tcs.TrySetResult(await Route(cmd, ct)); }
            catch (Exception ex) { CrashLog.Write("interactive-route", ex); tcs.TrySetResult(CommandResult.Fail("internal", ex.Message)); }
        });
        return tcs.Task;
    }
