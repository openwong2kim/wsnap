...
7. **Phase 6 (남은 전부)**: Avalonia 상주 앱 완성 — WPF `App.cs`/`App.Control.cs` 대응(트레이 아이콘(WinForms 유지)+핫키 디스패치+IResidentHost 구현+파이프 서버 시작+SingleInstance), **파이프 경로 보안 재검증 반복**, **실기기 멀티모니터 mixed-DPI 검증(§3-d, 사용자 하드웨어 필요)**, AssemblyName `wsnap` 복귀+release.yml 전환, 최소 1릴리즈 preview 옵트인 후 기본 전환.
