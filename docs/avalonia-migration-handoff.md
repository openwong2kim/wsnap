...
| [#50](https://github.com/openwong2kim/wsnap/pull/50) | **Phase 2**: CaptureOverlay Avalonia 이식(4-rect 딤) + ClipboardWatcher 프레임워크-무관 재작성 + MonitorPlacement/ScreenGrab 무관화·링크 + AvImaging/Icons | MERGED |
| [#52](https://github.com/openwong2kim/wsnap/pull/52) | **Phase 3a**: Toast+ThumbnailWindow 이식, ToastStub 삭제, ClipboardCore.CopyImageFile/CopyTextSuppressed | MERGED |
| [#53](https://github.com/openwong2kim/wsnap/pull/53) | **Phase 3b**: HistoryWindow+SettingsWindow 이식(언어 라이브 프리뷰, VkFromKey, ConfirmDialog) | MERGED |
| [#54](https://github.com/openwong2kim/wsnap/pull/54) | **Phase 4**: 레코더 3종 프레임워크-무관화(RecorderUi 배지 추상화) + Control층 전체 링크 + 헤드리스 CLI/MCP + 보안 재검증 | MERGED |
| [#55](https://github.com/openwong2kim/wsnap/pull/55) | **Phase 5**: EditorWindow 이식(12개 도구·undo/redo·크롭) + Thumbnail/History 편집 버튼 복원 | MERGED |
| [#57](https://github.com/openwong2kim/wsnap/pull/57) | **Phase 6 전반부**: Avalonia 상주 앱 배선(핫키+WinForms 트레이+워처+옵트인 파이프+SingleInstance) + 파이프 보안 재검증 | OPEN |
...
3. ~~**Phase 2**~~ **완료(PR #50 MERGED)**: 실기기 mixed-DPI 재검증은 사용자 지시로 **이론 분석으로 대체**(§3-d-보완 참조, 하드웨어 확보 시 실검증은 Phase 6 컷오버 전 필수로 이월).
...
4. **Phase 3 — 완료(3a=PR #52 MERGED, 3b=PR #53 MERGED)**: **3a = Toast+ThumbnailWindow 이식 완료** — `Wsnap.Avalonia/Toast.cs`(Transitions 기반 페이드/슬라이드, MonitorPlacement 물리 배치, ScalingChanged 재배치; **ToastStub.cs 삭제됨** — 링크된 Ocr.cs 진행 메시지가 실제로 표시됨), `Wsnap.Avalonia/ThumbnailWindow.cs`(스택/핀/자동해제/클릭 복사/Ctrl+클릭 경로 복사/우드래그 플링/드래그아웃 — IStorageFile은 스파이크(c)대로 **사전 해석**, `_filePath` 변경 시 재해석; 축소 디코드는 SKCodec 헤더 + `Bitmap.DecodeToWidth/Height`; **편집 버튼은 Phase 5의 EditorWindow 이식 때 추가** — 의도적 차이). 공유 정리: `ClipboardCore.CopyImageFile`/`CopyTextSuppressed` 신설, WPF `ImageClipboard.CopyImageFile`은 위임. 검증: 외부 하네스 — 물리 배치 정확(작업영역 우하단 @125%), 4사분면 픽셀 정확, 클릭→클립보드 PNG(크기·픽셀 정확), Ctrl+클릭→경로 텍스트, 플링→해제·앱 종료, Toast 시각 확인; `.smoke` 11/11.
   **3b = HistoryWindow+SettingsWindow 이식 완료(PR #53 MERGED)**: `HistoryWindow`(갤러리 타일+호버 액션바+컨텍스트메뉴+타일 클릭 복사/Ctrl+클릭 경로/드래그아웃 — IStorageFile은 **타일 hover 시 사전 해석**; 삭제 확인은 자체 `ConfirmDialog` — Avalonia에 MessageBox 없음; **재편집은 Phase 5에서**), `SettingsWindow`(전체 카드 이식: 언어 라이브 프리뷰 rebuild 구조 그대로, OCR 칩+다운로드 진행률, 핫키 캡처는 tunnel-routed KeyDown + **자체 `VkFromKey` 매핑**(WPF KeyInterop 대체 — 훅이 raw VK를 저장하므로), 폴더 픽커는 StorageProvider). 검증: 외부 하네스 — 히스토리 타일 렌더(적/청 픽셀 스캔), 타일 클릭→클립보드 PNG 픽셀 정확, ESC 종료; 설정 토큰 3종(창배경/Panel 카드/accent Save 버튼) 픽셀 존재, WM_CLOSE 클린 종료(설정 저장 없음 — 사용자 config 무변경), 두 창 스크린샷 시각 확인(한국어 로컬라이즈 포함). `.smoke` 11/11.
5. ~~**Phase 4**~~ **완료(PR #54 MERGED)**: 레코더 3종을 복제가 아니라 **프레임워크-무관화** — WPF 의존(배지 창·Int32Rect·Dispatcher)을 추출: ①`RecorderUi.cs`(신규 공유) — `IRecorderBadge` + 호스트 등록 `BadgeFactory`(WPF=`RecorderBadgeWpf.cs`, Avalonia=`Wsnap.Avalonia/RecorderBadge.cs`, 물리픽셀 top-center 배치·자체 마셜링), ②region 타입 `Int32Rect`→`System.Drawing.Rectangle`(WPF 호출부 App.cs/App.Control.cs 변환), ③GifRecorder 타이머는 `Start()` 시점 SynchronizationContext 캡처+Post(스레딩 모델 동일, 재진입 가드). 세 레코더 모두 Avalonia에 링크. **Control층 전체 링크**(CaptureCore/CliRouter/CommandRouter/ControlGate) + Avalonia `Program.Main`에 WPF와 동일한 헤드리스 CLI/MCP 분기(`wsnap-avalonia <verb>`/`mcp`, PipeClientRouter 위임 포함). **보안 재검증**: 게이트가 외부제어 off(기본)에서 CLI 캡처 거부(exit 1, "disabled") 실측, Avalonia 앱 실행 중 `wsnap.control.v1` 파이프 부재 확인(옵트인 없이는 서버 미생성 — 제로 어택서피스 유지), `--help` 정상. GIF E2E: 색상 교차 창 3초 녹화 → 30프레임/300×200/딜레이 10cs/무한루프/양 색상 캡처 정확. WPF 회귀 `.smoke` 11/11. **잔여: 상주(트레이) 호스트 배선(IResidentHost 구현+파이프 서버 시작)은 Phase 6 컷오버 시 — 그때 파이프 경로 보안 재검증 반복 필수.**
6. ~~**Phase 5**~~ **완료(PR #55 MERGED)**: `Wsnap.Avalonia/EditorWindow.cs` 전체 이식 — 12개 도구(선택/화살표/직선/사각/원/펜/형광/텍스트/번호/모자이크/흐림/자르기), undo/redo op 스택, 클립보드 붙여넣기/파일 드롭, 단축키 전부. **이식 결정 3건**: ①화살표/직선 Path는 지오메트리를 자기 bounds 원점 기준 **상대 좌표**로 만들고 Canvas.Left/Top으로 배치(WPF의 frozen 지오메트리 클론-트랜스폼 이동은 Avalonia Geometry API에 없음 — Translate가 단일 경로로 통일), ②크롭 딤은 4-rect(스파이크(a)), ③RenderFinal은 Avalonia `RenderTargetBitmap`(96 DPI 1:1 — 제품 래스터라이저, 화면 검증 아님) + 크롭은 SkiaSharp `ExtractSubset`. Phase 3에서 유보한 Thumbnail/History **편집 버튼 복원**. 검증(외부 SendInput): 화살표 드래그→Ctrl+S 저장 PNG 600×400+빨강 271px 베이크, 화살표→Ctrl+Z→저장 시 빨강 0(언두 동작), 크롭 도구(C)+드래그+Enter→축소 출력(152×99), 에디터 UI 스크린샷 시각 확인(도구 토글/스와치/버튼 한국어). `.smoke` 11/11.
7. ~~**Phase 6 전반부 — 상주 앱 배선**~~ **완료(PR #57)**: Avalonia bare 실행 = 완전한 상주 앱. `Wsnap.Avalonia/App.Resident.cs`(WPF App.cs 이식: 핫키 디스패치, WinForms NotifyIcon+TrayMenuTheme 트레이(의도적 유지 섬), Clipboard/Folder 워처, 옵트인 파이프, 업데이트 체크, 메모리 트림, 프리웜, 트레이 캡처 진입점 — 원샷 모드는 CaptureCore+PresentCapture 경유) + `App.Control.cs`(IResidentHost 이식, Avalonia 디스패처 TCS 브리지) + Program.Main에 **WPF와 동일 뮤텍스** SingleInstance(재실행=캡처 시그널, 두 상주가 동시 실행 불가 — 의도). FolderWatcher 프레임워크-무관화(SynchronizationContext 캡처+ClipboardCore). csproj: 아이콘·로케일·OCR det/cls 모델 임베드+PerMonitorV2 매니페스트. 상주 배선은 `Dispatcher.UIThread.Post`로 **지연 실행**(OnFrameworkInitializationCompleted 시점엔 AvaloniaSynchronizationContext 미설치 가능 — 훅/워처의 SyncContext 캡처가 루프 기동 후여야 안전). 검증(전부 외부): 기동 프로브, 핫키→드래그→160×120 물리픽셀 정확, 2차 실행 시그널, WinForms 다크 트레이 메뉴가 Avalonia 루프에서 렌더(픽셀 1316/1500 다크), **파이프 보안 재검증**(기본 off=파이프 부재+CLI 거부 resident_required+파일 미생성 / 옵트인=파이프 생성+`capture --full` 위임 1920×1080 정확+외부 호출자 content redacted), `.smoke` 11/11. 검증 하네스는 `--resident-demo[=pipe]`/`--tray-menu-probe`(인메모리 설정 샌드박스).
8. **Phase 6 잔여 (컷오버, 전부 사용자 결정/하드웨어 대기)**: ①**실기기 멀티모니터 mixed-DPI 검증**(§3-d, 사용자 하드웨어 필요 — 블로킹), ②AssemblyName `wsnap` 복귀+release.yml 전환(§5 함정 참조: `.smoke` CS0579 재발 주의 — 컷오버 커밋에서만), ③최소 1릴리즈 preview 옵트인 후 기본 전환.
...
- **DPI-unaware 프로세스의 `SetCursorPos`/`keybd_event` 하네스는 좌표가 125% 가상화됨** — Phase 6 검증에서 실측(기대 160×120이 정확히 1.25×인 200×150로 저장돼 앱 버그로 오인할 뻔). PowerShell 인라인 하네스는 주입 전에 반드시 `SetProcessDPIAware()`를 호출할 것. 앱 자체는 물리픽셀 정확했음.
- **OnFrameworkInitializationCompleted 시점엔 Avalonia의 SynchronizationContext가 아직 미설치일 수 있음** — SyncContext를 캡처하는 컴포넌트(HotkeyHook/FolderWatcher)와 메시지-온리 창(ClipboardWatcher)의 생성은 `Dispatcher.UIThread.Post`로 지연할 것(App.Resident.cs StartResident가 이 패턴).
